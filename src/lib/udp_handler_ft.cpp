...
void UDPHandler<DOF>::receiveLoop() {
    ...
    while (!stop_threads) {
        ...
        std::memcpy(received_ft.data(), buffer + 2*(sizeof(double) * DOF), sizeof(double) * DOF);

        {
            std::lock_guard<std::mutex> lock(state_mutex);
            latest_received = ReceivedData{received_jp, received_jv, received_ft, std::chrono::steady_clock::now()};
        }
    }
    recv_socket.close();
}
...
template <size_t DOF>
void UDPHandler<DOF>::send(const jp_type& jp, const jv_type& jv, const ft_type& ft) {
    {
...
void UDPHandler<DOF>::sendLoop() {
    ...
    while (!stop_threads) {
        ...
        jv_type data_to_send_jv = pending_send_jv;
        ft_type data_to_send_ft = pending_send_ft;
        lock.unlock();
        ...
        std::memcpy(buffer + 2*(sizeof(double) * DOF), data_to_send_ft.data(), sizeof(double) * DOF);
